...
- `Metro_Detroit_Lead_Gen_System.xlsx` — the pipeline CRM. Tabs: Read Me (full algorithm writeup + sources, incl. how larger companies source/advertise for this work), Channel Performance (the adaptive scoring engine, 10 channels), Dashboard (live formulas), Pipeline (master prospect tracker, 32 seeded leads), Weekly Call Rotation, Events (in-person networking calendar).
...
A scheduled cloud agent runs weekly (Mondays, 6am ET) against this repo, with the `Google_Calendar` MCP connector attached. Each run:
